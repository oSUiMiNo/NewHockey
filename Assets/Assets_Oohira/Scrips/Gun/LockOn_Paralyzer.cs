...
public class LockOn_Paralyzer : LockOn
{
    ...
    {
        return gunShoot.paralyzer;
    }
    protected override void Difine_BurrelLocation()
    {
        burrel_Location = burrel_Location = GameObject.Find("Barrel_Location_Paralyzer");
    }
    protected override void Difine_WhileLockOn(Collider other)
    {
        whileLockOn = (other.gameObject.tag == "Bullet1");
    }
    ...
    protected override void UpdateLockOn()
    {
        ...
        if (OVRInput.GetDown(OVRInput.Button.PrimaryIndexTrigger, OVRInput.Controller.Touch) || Input.GetMouseButtonDown(0))
        {
            ...
            //pool_Bullet.Object_Hide(target_Narrowed);
            //pool_LockOnOverlay.Object_Hide(lockOnOverlays_Ball[key_Target]);
            StopLockOn(target_Narrowed.GetComponent<Collider>());
        }
    }
